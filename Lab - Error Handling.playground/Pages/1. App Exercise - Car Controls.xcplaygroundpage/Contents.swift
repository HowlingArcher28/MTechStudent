...
func turnOnClimateControl(temp: Int) throws {
    if engineRunning == false {
        try startEngine(gallonsGasInTank: 2)
    }
...
do {
    ...
    print("Climate control set to \(currentTemperature)°")
} catch {
    print("Failed to turn on climate control: \(error)")
}
...
func toggleDoorLocks() throws{
    guard selectedGear == .park else {
        throw RemoteError.notInPark
    }
    locked.toggle()
    print("Doors are now \(locked ? "locked" : "unlocked")")
}
...
do {
    ...
    print("Could not toggle door locks: \(error)")
}
