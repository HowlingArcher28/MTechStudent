...
} catch CommunicationError.timedOut(waitTime: let waitTime) {
    print("Communication timed out after \(waitTime) seconds.")
    
} catch CommunicationError.cancelledByUser {
    print("Communication cancelled by user.")
    
} catch CommunicationError.rejectedByServer(reason: let reason) {
    print("Communication rejected by server: \(reason)")
