...
if let calculated = calculateArea(x: 5.3, y: 7.7) {
    print(calculated)
}
if let calculation = calculateArea(x: -5.8, y: 56.7) {
    print(calculation)
}
...
func add(optionalOne: Int?, optionalTwo: Int?) -> Int? {
    guard let one = optionalOne, let two = optionalTwo else {
        return nil
    }
    return one + two
}

let sum1 = add(optionalOne: 3, optionalTwo: 7)
let sum2 = add(optionalOne: nil, optionalTwo: 8)
/*:
