...
struct WatchmakersToolbox: Toolbox {
    ...
    func listTools() -> [WatchmakerTools] {
        [WatchmakerTools(), WatchmakerTools()]
    }
}

let mechanic = MechanicToolbox()
let art = ArtToolbox()
let watchmaker = WatchmakersToolbox()
let mechanicTools = mechanic.listTools()
let artTools = art.listTools()
let watchmakerTools = watchmaker.listTools()
...
struct FoodCourier: DeliveryService {
    typealias Parcel = Meal
    func deliver(parcel: Meal) {
        print("Delivering meal: \(parcel.name)")
    }
}
...
struct MailCourier: DeliveryService {
    ...
    func deliver(parcel: Letter) {
        print("Delivering letter to: \(parcel.recipient)")
    }
}

let foodCourier = FoodCourier()
foodCourier.deliver(parcel: Meal(name: "Pasta Primavera"))

let mailCourier = MailCourier()
mailCourier.deliver(parcel: Letter(recipient: "Taylor Swift"))
