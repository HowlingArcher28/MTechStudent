...
let getUser = GetUserRequest(username: "alice")

Task {
    do {
        let user: User = try await service.performRequest(getUser)
        print("Fetched user: \(user.username)")
        
    } catch {
        print("GetUserRequest failed: \(error)")
    }
}
...
let getRecentWorkout = GetRecentWorkoutRequest()

Task {
    do {
        let lastWorkout: Workout = try await service.performRequest(getRecentWorkout)
        print("Fetched recent workout: \(lastWorkout)")
        // Answer: The function returns Workout, from `GetRecentWorkoutRequest.Response` (its associatedtype via `typealias Response = Workout).
    } catch {
        print("GetRecentWorkoutRequest failed: \(error)")
    }
}
/*:
